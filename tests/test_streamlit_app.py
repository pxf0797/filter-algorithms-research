"""Tests for browse.app.py pure functions.
...
class TestDateMarkers:
    ...
    def test_daily_markers_monday_boundary(self):
        ...
        pos, labels = browse.app._date_markers(dates, "日线")
        assert len(pos) > 0
        # weekday: 0=Monday
    ...
    def test_weekly_markers_month_boundary(self):
        dates = pd.date_range("2026-01-01", periods=120, freq="D")
        pos, labels = browse.app._date_markers(dates, "周线")
        assert len(pos) > 0
        for idx in pos:
            if idx > 0:
    ...
    def test_monthly_markers_january(self):
        ...
        pos, labels = browse.app._date_markers(dates, "月线")
        assert len(pos) > 0
        for idx in pos:
    ...
    def test_quarterly_same_as_monthly(self):
        dates = pd.date_range("2026-01-01", periods=365, freq="D")
        pos_q, labels_q = browse.app._date_markers(dates, "季线")
        pos_m, labels_m = browse.app._date_markers(dates, "月线")
        assert pos_q == pos_m
...
class TestComputeFilters:
    def test_unknown_filter_id(self):
        ...
        filtered, filtered2 = browse.app._compute_filters(noisy, t, cfg)
    ...
    def test_sma_filter(self, simple_noisy):
        ...
        filtered, filtered2 = browse.app._compute_filters(simple_noisy, t, cfg)
        assert filtered2 is None
    ...
    def test_filtered_is_float_raveled(self, simple_noisy):
        t = np.arange(6, dtype=float)
        cfg: Dict[str, Any] = {"_fid": "sma", "pv": {"window": 3}, "_dual": False}
        filtered, _ = browse.app._compute_filters(simple_noisy, t, cfg)
...
class TestComputeSchmittTrigger:
    def test_disabled_when_show_sch_false(self):
        ...
        result = browse.app._compute_schmitt_trigger(filtered, t, cfg)
    ...
    def test_tight_params_more_nonzero(self, sine_signal):
        ...
        tight_result = browse.app._compute_schmitt_trigger(filtered, t, tight_cfg)
        loose_result = browse.app._compute_schmitt_trigger(filtered, t, loose_cfg)
...
class TestComputePredictionPairs:
    def test_empty_when_show_pred_false(self):
        ...
        result = browse.app._compute_prediction_pairs(
    ...
    def test_empty_when_schmitt_none(self):
        ...
        result = browse.app._compute_prediction_pairs(t, filtered, None, cfg, [(0, 10)])
        assert result == []
    ...
    def test_parabolic_fit_on_valid_pairs(self):
        ...
        result = browse.app._compute_prediction_pairs(t, filtered, schmitt, cfg, all_pairs)
        assert len(result) > 0
        for r in result:
...
class TestDetermineSubplotLayout:
    def test_minimal_no_features(self):
        rows, rh, titles, mr, rr, vr, sar, ssr, ar, pnl_row, cross_row, align_row = (
            browse.app._determine_subplot_layout(False, False, False, False, "周线")
        )
        assert rows == 4
        assert len(rh) == 4
        assert len(titles) == 4
    ...
    def test_with_schmitt_no_strategy(self):
        rows, rh, titles, mr, rr, vr, sar, ssr, ar, pnl_row, cross_row, align_row = (
            browse.app._determine_subplot_layout(True, False, False, False, "周线")
        )
        assert rows == 5
    ...
    def test_full_layout_all_features(self):
        rows, rh, titles, mr, rr, vr, sar, ssr, ar, pnl_row, cross_row, align_row = (
            browse.app._determine_subplot_layout(True, True, True, True, "周线")
        )
        assert rows == 8
    ...
    def test_cross_pnl_no_alignment(self):
        rows, rh, titles, mr, rr, vr, sar, ssr, ar, pnl_row, cross_row, align_row = (
            browse.app._determine_subplot_layout(True, True, True, False, "周线")
        )
        assert rows == 7
    ...
    def test_strategy_no_cross_no_alignment(self):
        rows, rh, titles, mr, rr, vr, sar, ssr, ar, pnl_row, cross_row, align_row = (
            browse.app._determine_subplot_layout(True, True, False, False, "60分钟")
        )
        assert rows == 6
        assert pnl_row == 6
    ...
    def test_no_schmitt_ar_is_fourth(self):
        rows, rh, titles, mr, rr, vr, sar, ssr, ar, pnl_row, cross_row, align_row = (
            browse.app._determine_subplot_layout(False, False, False, False, "日线")
        )
        assert ar == 4
        assert sar is None and ssr is None
