...
**实际测试计数**：328 个测试 / 12 个测试文件 / 65 个测试类 (+ 6 个独立函数)

pytest 输出：`pytest tests/ --collect-only -q` 确认 328 个测试被收集。
...
├── test_preset_ui_actions.py        # 预设管理 UI 行为测试（40 测试，7 类）
...
| 4 | `test_boundary.py` | 9 | 21 | 6.4% | `slow` |
...
| test_preset_ui_actions.py | 7 个类 | 40 | action 标志、名称同步、preset_map、category、toast、widget 冲突 |

---

## 4. Bug-修复-测试追溯

| 修复 ID | Bug 描述 | 修复模块 | 覆盖测试 | 测试文件 |
|:--|:--|:--|:--|:--|
| P0-1 | `import_json_files_as_presets` 返回 `int` 而非 `(int, list)` 元组 | config_db.py | CDB-44~CDB-46 | test_config_db.py |
| P1-1 | `delete_preset` 不返回操作结果 | config_db.py | CDB-37~CDB-39 | test_config_db.py |
| P1-2 | `rename_preset` 重名时抛异常而非安全返回 None | config_db.py | CDB-12, CDB-40 | test_config_db.py |
| P1-5 | `save_preset` 未校验 JSON 有效性 | config_db.py | CDB-14, CDB-33, CDB-34 | test_config_db.py |
| P1-6 | `rename_preset` 空名称未校验 | config_db.py | CDB-41 | test_config_db.py |
| P0-2 | 删除被 ticker 引用的预设级联删除而非 SET NULL | config_db.py | CDB-39 | test_config_db.py |
| — | `check_data_health` 未收集所有周期的 issue | db.py | test_health_issues_per_tf_bug_fix | test_db.py |
...
- **65 个测试类**：分布在 10 个类式测试文件中（`test_integration.py` 不含类）
- **6 个独立函数**：`test_integration.py` 的 6 个顶级函数
- **328 个测试**：来自 `pytest --collect-only -q` 的精确计数
