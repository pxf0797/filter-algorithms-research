...
- `filter/db.py` 提供 `DB_PATH`（`data/market.db`）和 `get_conn()`（WAL 模式，`sqlite3.Row` factory）
...
| 改动位置 | 行号范围 | 内容 | 行数 |
|:--|:--|:--|:--:|
| `filter/streamlit_app.py` import 区 | ~顶部 | `from config_db import *` | +1 |
...
| 新建文件 | `filter/config_db.py` | 模块文件 | ~120 |
...
| 风险 | 措施 |
|:--|:--|
| 新建 `config/` 目录与 Streamlit 内置 `config.py` 冲突 | 模块文件放在 `filter/` 下，命名为 `config_db.py`（不叫 `config.py`）|
