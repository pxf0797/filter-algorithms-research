...
| Lag | 通过脉冲响应质心法/理论群延迟确定的滤波器延迟（样本数） | ↓ 越小越好 |
...
|:----:|--------|------|---------|---------|----------|-----------|---------|:---------:|
| 1 | **Gauss** | 0.093 | +3.20 | 0.0 | 4.1 | <0.001 | 0.966 | **0.960** |
| 2 | **SMA** | 0.130 | +2.90 | 5.0 | 7.7 | <0.001 | 0.954 | **0.870** |
| 3 | Butter | 0.100 | +1.90 | 0.0 | 19.8 | <0.001 | 1.078 | 0.788 |
| 4 | SavGol | 0.117 | +2.17 | 5.0 | 23.3 | <0.001 | 1.015 | 0.783 |
| 5 | WMA | 0.160 | +1.62 | 5.0 | 13.6 | <0.001 | 0.991 | 0.746 |
| 6 | EMA | 0.154 | +1.80 | 4.0 | 20.7 | <0.001 | 1.027 | 0.738 |
| 7 | Median | 0.107 | +2.88 | 2.0 | 109.1 | <0.001 | 1.143 | 0.700 |
| 8 | ALMA | 0.225 | +1.01 | 16.0 | 3.9 | <0.001 | 1.020 | 0.536 |
| 9 | Kalman | 0.124 | +0.44 | 1.4 | 214.9 | 0.007 | 0.990 | 0.533 |
| 10 | LOWESS | 0.317 | +0.16 | 0.0 | <0.001 | 0.021 | 0.916 | 0.500 |

**核心观察：**

1. **零相位滤波器整体占优。** 排名前三均为零相位滤波器（Gauss、Butterworth filtfilt、Savitzky-Golay），在离线批处理场景下不引入相位失真的滤波器具有系统性优势。延迟对因果滤波器的惩罚显著（SMA、WMA 均因 5 样本延迟而扣分）。

2. **Gauss 排名第一**得益于平滑的频域滚降（无旁瓣）、零相位、以及所有维度均无短板的均衡表现。它是一种优秀的通用离线滤波器。

3. **SMA 以短窗口 (n=11) 获第二名**，彰显了简单方法的有效性。注意：窗口增大时 SMA 延迟成比例增长（$\tau = n/2$），各项指标将快速劣化。

4. **Butterworth 升至第三**，其零相位特性（filtfilt）和可设计的陡峭截止是主要优势。粗糙度 (19.8) 略高，可减小截止频率或增加阶数改善。

5. **Savitzky-Golay 排名第四**，粗糙度 (23.3) 高于 Gauss 和 SMA，但这并非缺陷——它有意保留信号峰值和面积信息；粗糙度较高恰好反映了特征保持能力。

6. **ALMA 延迟最大 (16.0)**，源于其偏移高斯窗口 (offset=0.85) 将权重重心后移以换取极优平滑度（粗糙度 3.9，仅次于 LOWESS）。这是一种有意的延迟-平滑度权衡，验证了报告 §3.5 中 offset 参数的核心机制。

7. **Median 粗糙度最高 (109.1)**，因其边缘保持特性：不模糊阶跃和尖峰。在 Impulse 信号上 MSE 最低印证了对脉冲噪声的免疫能力。

8. **Kalman 排名靠后**，因默认参数 $(Q=0.01, R=0.1)$ 未针对测试信号调优。Kalman 的优势在于状态估计和不确定性量化，而非通用信号平滑。
