...
**Architecture:** 新增 3 个纯函数（区间扫描、多项式拟合、trace 构建）+ 1 个 UI checkbox + `_render_chart` 内一小段集成代码。全部改动集中在 `filter/streamlit_app.py` 单文件（当时为单文件架构），约 100 行新增。
...
|------|--------|----------------|
| `filter/streamlit_app.py` | Modify (+~100 lines) | 新增预测曲线函数 + UI + 图表集成 |
...
**Files:**
- Modify: `filter/streamlit_app.py` — insert 3 new functions after `_schmitt_trigger()` (~line 613)
...
**Files:**
- Modify: `filter/streamlit_app.py` — `_render_params()` Row 1 columns (~line 631)
...
**Files:**
- Modify: `filter/streamlit_app.py` — `_render_chart()`, Schmitt + figure section (~line 777)
...
```bash
cd /Users/xfpan/claude/filter_research && streamlit run filter/streamlit_app.py --server.port 8502
```
...
git add filter/streamlit_app.py
