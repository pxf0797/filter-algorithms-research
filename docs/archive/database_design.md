...
**当前流程**（`filter/streamlit_app.py` 行 436-510，原单文件版本）：
...
**当前流程**（`filter/streamlit_app.py` 行 513-548，原单文件版本）：
...
**当前**（`filter/streamlit_app.py` 行 970-980，原单文件版本）：
...
| 文件 | 改动 | 行数变化 | 说明 |
|------|------|----------|------|
| **新增：`filter/db.py`** | 新建 | +~90 | SQLite 数据访问层 |
| `filter/streamlit_app.py:_fetch_stock` | 改造 | -15 +12 | yfinance → upsert DB → query DB 返回；移除 parquet 保存 |
| `filter/streamlit_app.py:_sync_to_display` | 改造 | -12 +10 | archive parquet 读取 → db.query_kline() |
| `filter/streamlit_app.py:main` (日期范围) | 改造 | -6 +4 | parquet 读取 → db.get_date_range() |
| `filter/streamlit_app.py` 顶部 import | 修改 | +1 | 新增 `import db` |
...
| 步骤 | 操作 | 命令 | 预期结果 |
|------|------|------|----------|
| 1 | 创建 `db.py` | 写入 `filter/db.py` | 模块可 import |
...
| 6 | 修改 `filter/streamlit_app.py` | 改造 `_fetch_stock`, `_sync_to_display`, `main` 中的日期查询 | 改动清单见 3.5 |
| 7 | 启动 Streamlit 验证 | `streamlit run filter/streamlit_app.py` | 图表正常渲染，数据正确 |
...
**任务**：
1. 创建 `filter/db.py`（约 90 行）
2. 修改 `filter/streamlit_app.py`:
   - 顶部新增 `import db`
