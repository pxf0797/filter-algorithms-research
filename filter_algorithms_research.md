...
**群延迟：** EMA 的相位非线性——在 DC 处群延迟为 $\tau_g(0) = (1-\alpha)/\alpha$（近似 $(n-1)/2$），但在高频处延迟更短。这使得 EMA 对近期变化反应比 SMA 更快，但代价是不同频率成分之间的相位关系被扭曲。
...
3. **Sigma** $\sigma = n / \text{sigma}$：控制高斯包络的展宽程度，sigma 越大，权重分布越广，平滑度越高。默认值 6.0。
...
def alma(series, n, offset=0.85, sigma=6.0):
    ...
    for t in range(n - 1, len(series)):
        ...
        result[t] = np.dot(window[::-1], weights)
    return result
...
| **每步复杂度** | O($n^3$) | O(阶数) | O($N\sigma$) | O($Nk$) | O($N^2$) | O($T$) | **O(1)** | **O(1)** | O($Nn$) |
