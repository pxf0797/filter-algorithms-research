...
# ---------------------------------------------------------------------------
# Layer 1：PnL 反馈驱动持仓（二值门控 overlay）
# 纯 PnL 反馈：对理论权益曲线的回撤做二值迟滞门控，不改 Layer 0、不需 filtered 价格。
# 设计见 docs/backtesting/pnl-feedback-position-process-v1.md
# ---------------------------------------------------------------------------
def _pnl_feedback_gate_1d(shadow, dd_clear, dd_recover):
    """对单方向理论权益曲线 shadow 应用「回撤迟滞二值门控」。

    - shadow: np.ndarray，某方向理论权益（long_pnl 或 short_pnl），含未实现，逐 bar。
    - dd_clear: 回撤越过此值 → 清仓（ACTIVE→FLAT）。设为 inf 即永不清仓（零回归）。
    - dd_recover: 回撤回落到此值内 → 恢复（FLAT→ACTIVE）。须 < dd_clear，构成迟滞防抖。

    Returns (active, actual, clears, recovers):
      active  : np.ndarray[bool]  每 bar 是否持仓（True=仓 / False=清仓）
      actual  : np.ndarray[float] 实际权益：ACTIVE 跟随 shadow，FLAT 水平冻结
      clears  : List[int] 触发清仓的 bar 索引
      recovers: List[int] 触发恢复的 bar 索引
    """
    n = len(shadow)
    ...
    clears, recovers = [], []
    if n == 0:
        return active, actual, clears, recovers
    ...
    for i in range(n):
        ...
        if s > peak:
            peak = s
        dd = 1.0 - s / peak if peak > 0 else 0.0

        if is_active and dd > dd_clear:
            is_active = False
            clears.append(i)
        elif (not is_active) and dd < dd_recover:
            is_active = True
            recovers.append(i)

        active[i] = is_active
        if is_active:
            if not prev:                      # 刚恢复：平移使实际线从冻结值续接
                offset = frozen - s
            actual[i] = s + offset
        else:
            if prev:                          # 刚清仓：冻结在清仓前一 bar 的实际值
                frozen = actual[i - 1] if i > 0 else shadow[0]
            actual[i] = frozen
        prev = is_active
...
def _compute_pnl_feedback_positions(long_pnl, short_pnl,
                                    dd_clear=0.08, dd_recover=0.03):
    """Layer 1 — 纯 PnL 反馈的二值持仓 overlay。

    对 long_pnl / short_pnl **各自独立**应用回撤迟滞门控（多空互不影响，
    对齐现有双独立曲线）。不修改 Layer 0，不需要 filtered 价格。
    连亏触发（T2/LOSS_K）为未来可选扩展，本版本仅做回撤触发（T1）。

    - dd_clear / dd_recover: 百分比小数（0.08 = 8%）。dd_clear=inf 即整体零回归
      （actual ≡ 理论曲线，逐字节复现 Layer 0）。

    Returns dict:
      actual_long / actual_short   : np.ndarray  gated 实际权益曲线
      long_active / short_active   : np.ndarray[bool]  gate 时间线（True=仓）
      long_clears / long_recovers  : List[int]
      short_clears / short_recovers: List[int]
    """
    long_pnl = np.asarray(long_pnl, dtype=float)
    short_pnl = np.asarray(short_pnl, dtype=float)
    l_active, l_actual, l_clears, l_recovers = _pnl_feedback_gate_1d(
        long_pnl, dd_clear, dd_recover)
    s_active, s_actual, s_clears, s_recovers = _pnl_feedback_gate_1d(
        short_pnl, dd_clear, dd_recover)
    return {
        "actual_long": l_actual, "actual_short": s_actual,
        "long_active": l_active, "short_active": s_active,
        "long_clears": l_clears, "long_recovers": l_recovers,
        "short_clears": s_clears, "short_recovers": s_recovers,
    }
