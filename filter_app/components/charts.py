...
(function() {{
    ...
    var _chartEl = document.getElementById('{div_id}');
    var _initialized = false;

    function tryInit() {{
        if (_initialized) return true;
        if (typeof Plotly === 'undefined') return false;
        if (window._plotlyCdnFailed) delete window._plotlyCdnFailed;
        _initialized = true;
        ...
        Plotly.newPlot('{div_id}', figure.data, figure.layout, config).then(function(gd) {{
            ...
            _chartEl.style.display = 'none';
            _fallbackEl.style.display = 'block';
            _fallbackEl.innerHTML = '<p>图表渲染失败: ' + (err.message || '未知错误') + '</p>';
        }});
        return true;
    }}
    ...
    if (!tryInit()) {{
        ...
        var _pollTimer = setInterval(function() {{
            ...
            }} else if (_pollAttempts >= _pollMax) {{
                clearInterval(_pollTimer);
                _chartEl.style.display = 'none';
                _fallbackEl.style.display = 'block';
            }}
