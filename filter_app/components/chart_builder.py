"""
图表构建模块 — 子图布局、trace/shape/annotation 生成函数

从 streamlit_app.py 提取的纯 Plotly 图表组装函数。
不包含 Streamlit UI 组件 — 仅返回可序列化为 go.Figure 的 dict 结构。

依赖: components.charts (prediction traces, cross PnL, alignment, holding bands, BS markers)
"""
